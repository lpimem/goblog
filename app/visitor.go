...
func loadVisitorCount() {
	...
	if err != nil {
		revel.AppLog.Errorf("Cannot load visitor count, %s", err.Error())
		VisitorCount = 0
	} else {
...
func RecordVisit(c *revel.Controller) revel.Result {
	...
	go func(t string, f string) {
		...
		revel.AppLog.Info(msg)
		if f, err := os.OpenFile(BaseDir+"/visitors_"+today,
			os.O_CREATE|os.O_APPEND|os.O_WRONLY,
			0644); err == nil {
			defer f.Close()
			if _, err = f.WriteString(msg); err != nil {
				revel.AppLog.Errorf("Cannot log visitor info, %s\r\n", err.Error())
			}
		} else {
			revel.AppLog.Errorf("Cannot log visitor info, %s\r\n", err.Error())
		}
		...
		if err != nil {
			revel.AppLog.Errorf("Cannot update visitor info: %s", err.Error())
		}
	}(target, from)
	return nil
}
